-- Migration 00013: Restrict update_order to QUEUED status only (before kitchen starts)
-- Customers can only edit their order while the restaurant has not started preparing it.
    ...
    IF v_options_json IS NOT NULL AND jsonb_typeof(v_options_json) = 'array' AND jsonb_array_length(v_options_json) > 0 THEN
      ...
      LOOP
        ...
        IF NOT FOUND OR NOT v_option.is_available THEN
          RAISE EXCEPTION 'ตัวเลือกเสริมบางรายการไม่พร้อมให้บริการ';
        END IF;
